...
type BirthdayType = {
  id: number;
  email: string;
  certificate_type: "unique" | "anniversary" | "birthday-card" | "birthday" | string;
  first_name: string;
  last_name: string;
  phone_number: string;
  note?: string;
  select_certificate?: "this certificate is for my birthday" | "this certificate is for someone else";
...
const BirthdayCertificateManagement  = () => {
  const [certificates, setCertificates] = useState<BirthdayType[]>([]);
  ...
  useEffect(() => {
    const fetchCertificates = async () => {
      setLoading(true);
      try {
        const data = await fetchAllCertificates(savedToken!, "birthday");
        console.log(data)
          ...
          <tbody className="divide-y divide-gray-200">
            {certificates.map((cert) => (
              <tr key={cert.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-2 py-3 text-sm text-gray-700 text-center">{cert.recipient_first_name || "-"}</td>
                <td className="px-2 py-3 text-sm text-gray-700 text-center">{cert.recipient_last_name || "-"}</td>
...
export default BirthdayCertificateManagement;
